...
| `BEIGNET_GUARDIAN_MAX_BYTES` | Content one served guardian set may store (its encoded rows; SQLite's overhead comes on top) before a write that would cross it is refused with `ERR_QUOTA_EXCEEDED` (default 268435456). Refuses, never deletes |
