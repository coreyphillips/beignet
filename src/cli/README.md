...
| `sendPaymentAsync(bolt11, maxFeeSats?, amountSats?, metadata?)` | `{ paymentHash, status: 'PENDING' }` | Fire-and-forget pay. Returns immediately. Poll `getPayment()` for settlement. Drain mode and the spending limits apply as they do to `payInvoice`, so submission can throw `SERVICE_DRAINING` or `SPENDING_LIMIT_EXCEEDED`. |
