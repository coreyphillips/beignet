...
| Direct-funding payer record | `direct-funding/sender/records.ts`, wallet-data key `df:payments` | D1 | The payer signs a transaction a stranger built, and once its witness is out the coin may be spent by a transaction it did not broadcast. So the attestation, the negotiated transaction, our own witness and the funding txid are written BEFORE the witness is emitted, and a failed write withholds the witness and refuses the call, which is the last moment refusing is free. The record also pins the coin, the offer bytes and the change script to the request, so a retry after a crash resumes the same offer rather than committing a second coin (the fork had neither: defects D6 and D7). Outside the journal, the way `jit:*` is: `RecoveryMutation` has no `metadata` variant, and a payer's own bookkeeping must fail its send rather than fence the node. Kill test: `recovery-phase7-direct-funding.test.ts` SIGKILLs a real process against a real SQLite file at three boundaries (before the persist, after it, after the witness) and re-runs it against the same file. |
| Direct-funding receiver offer sessions | `direct-funding/receiver/`, wallet-data key `df:requests` | D1 for the request and the attempt budget, D3 for the session | The request record (receipt preimage, encryption key, path secret, attempt count, busy mark, `paidBy`) is durable, because envelopes carrying it are already out there and because losing the attempt bookkeeping would let a duplicate offer after a restart start a second channel for one payment. The negotiation itself dies with the process: a restarted receiver answers a re-sent offer with the recorded receipt rather than resuming an exchange whose channel binding is gone. Kill test: the restore and replay cases in `direct-funding-receiver.test.ts` and `direct-funding-requests.test.ts` (issue #612). |
