...
Revision 16 (2026-09-18): the seed-only restore design lives in docs/SEED-ONLY-RESTORE.md (tracked by #909). It records what this protocol delivers today when a node holds nothing but its mnemonic and what it does not: only quorum mode resumes exactly; the peer-storage tier tolerates ZERO completed payment rounds of staleness, because the DLP arm that 5.4 names as the safety net is a disjunction over both counters, and a device lost inside the 60 second capsule refresh window comes back ERRORED and asking the peer to close rather than held; and three fund-safety holes exist independent of any tier (#905, the labelled force-close hatch on a row whose local commitment the peer has already revoked; #906, the channel key index restarting at 1 on a bare-seed boot; #907, an all-zeroes your_last_per_commitment_secret accepted above revocation number 0) plus one route bug (#908, POST /ffor/enforce passes no acceptStaleStateRisk). The design adds, without a new server: a homing pass for the first hop after a seed import, a closed key/value journal variant carrying the liquidity peer identity, a signed head beacon pushed on the counterparty connection before every barrier-class message with a signed acknowledgement from a beignet provider, a capsule version 2 header declaring that policy, a second holder through the existing trusted-peer hatch, and a two-stage lift of the restore hold in which the channel takes HTLCs again while every automatic local broadcast stays refused. Nothing in the guardian sections changes; 5.8 gains a peer-ordered row that is explicitly NOT a barrier mode and mints no wire-safety proof.
