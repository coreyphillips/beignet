...
describe('Interop: LND pays through a JIT intercept SCID (issue #594)', function () {
	...
	// invoice and collect its flat and proportional fees from LND instead.
	for (const feeMode of ['skim', 'hop'] as const) {
		it(`createJitInvoice settles a delivery with ${feeMode} fees`, async function () {
			alice = makeNode(`jit-lsp-${feeMode}-903`, {
				fundingProvider: new BitcoindFundingProvider(),
				jitReceive: {
					enabled: true,
					fundingBufferSats: 20_000n,
					...
					feePpm: feeMode === 'hop' ? 500 : 0
				}
			});
			bob = makeNode(`jit-wallet-${feeMode}-904`);
			const aliceId = alice.getNodeId();
			const bobId = bob.getNodeId();
			...
			expect(invoice.feeMode).to.equal(feeMode);
			if (feeMode === 'hop') {
				expect(
					bob.getInvoice(invoice.paymentHash.toString('hex'))!.jitFee
				).to.equal(undefined);
			}
			...
			const delivered =
				feeMode === 'hop'
					? PAYMENT_MSAT
					: PAYMENT_MSAT - OPENING_FEE_SAT * 1000n;
			const settled = Date.now() + 20_000;
			while (
			...
			expect(
				Number(bob.listChannels()[0].localBalanceMsat),
				'bob was credited the agreed delivery'
			).to.be.greaterThanOrEqual(Number(delivered));
			if (feeMode === 'hop') {
				expect(bob.listChannels()[0].localBalanceMsat).to.equal(PAYMENT_MSAT);
				expect(BigInt(payment.payment_route.total_fees)).to.equal(
					OPENING_FEE_SAT + (PAYMENT_MSAT * 500n) / 1_000_000_000n
				);
			} else {
				expect(
					Number(bob.listChannels()[0].localBalanceMsat),
